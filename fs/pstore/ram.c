...
int zte_get_ramoops_paraments(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "mem_type", &pdata->mem_type);
	if (ret) {
		dev_err(&pdev->dev, "Failed to find mem_type.\n");
		return ret;
	}

	ret = of_property_read_u32(node, "dump_oops", &pdata->dump_oops);
	if (ret) {
...
static int rmem_ramoops_device_init(struct reserved_mem *rmem, struct device *dev)
{
	...
	ramoops_pdata = kzalloc(sizeof(struct ramoops_platform_data),
				GFP_KERNEL);
	...
	ret = platform_device_add_data(pdev,(const void *)ramoops_pdata,
