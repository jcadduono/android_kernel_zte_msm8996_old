...
static struct proc_dir_entry *msm_lcd = NULL;
static char msm_lcd_name[MDSS_MAX_PANEL_LEN] = {0};
...
void mdss_dsi_panel_lcd_proc(struct device_node *node)
{
	...
	msm_lcd = proc_create("msm_lcd", S_IRUGO, NULL, &proc_ops);
	if (!msm_lcd)
		pr_err("LCD %s: proc_create panel information failed!\n",
			__func__);
}
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	pr_debug("LCD %s: level=%d -> new_level=%d\n", __func__, level, bl_level);
...
static char R_AID_config_5[] = {
	0xCB, 0x10, 0x01, 0x80, 0x00, 0x00, 0x80, 0x60, 0x00,
...
int mdss_dsi_read_R_AID_offset(struct mdss_dsi_ctrl_pdata *ctrl,
	...
	int ndx)
{
	...
	strlcpy(msm_lcd_name, pinfo->panel_name, MDSS_MAX_PANEL_LEN);
