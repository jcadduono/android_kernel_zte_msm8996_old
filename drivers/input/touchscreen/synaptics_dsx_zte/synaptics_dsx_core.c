...
#endif
#include "../ndt_stress/ndt_stress.h"
...
extern int ndt_stress_set_value(unsigned short x1,  unsigned short y1, unsigned char status);
extern int ndt_stress_get_value(unsigned int *value);
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			if (touch_count == 0) {//first point
				...
				*/
				ret = ndt_stress_set_value(x, y, 1);
				if (ret == 0)
					ndt_stress_get_value(&z);
				else
					z = wx;
			}else//other case
				z = wx;
