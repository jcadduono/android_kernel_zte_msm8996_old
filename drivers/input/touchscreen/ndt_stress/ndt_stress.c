...
static int ndt_stress_gpio_setup(int gpio, bool config, int dir, int state)
{
	...
	if (retval) {
		pr_err("Failed to get gpio %d (code: %d)",
			gpio, retval);
		return retval;
	}
	...
	if (retval)
		pr_err("Failed to set gpio %d direction", gpio);
...
static int parse_dt(struct device *dev, struct ndt_stress_board_data *bdata)
{
	...
	retval = of_property_read_u32(np, "ndt,irq-on-state", &value);
	if (retval < 0)
		bdata->irq_on_state = 0;
	else
		bdata->irq_on_state = value;

	pr_info("irq_on_state: %d\n", bdata->irq_on_state);
	...
	pr_info("power_gpio: %d, power_on_state %d\n",
	...
	if (of_find_property(np, "ndt,reset-gpio", NULL)) {
		...
		retval = of_property_read_u32(np, "ndt,reset-active-ms", &value);
		if (retval < 0)
			return retval;
	...
	pr_info("reset_gpio: %d, reset_on_state: %d, reset_active_ms %d, reset_delay_ms %d\n",
		bdata->reset_gpio, bdata->reset_on_state, bdata->reset_active_ms, bdata->reset_delay_ms);

	return 0;
}

static int ndt_stress_i2c_write(struct i2c_client *client,
				u16 addr, int len, void *txbuf )
{
	int ret;
	unsigned char* buf;
	struct i2c_msg msg;
	...
	if ((!txbuf) || (!len)) {
		pr_err("txbuf is null, len is 0. write 0x%04X\n", addr);
		return -EINVAL;
	}

	buf = kmalloc(len+1, in_atomic() ? GFP_ATOMIC : GFP_KERNEL);
	if (!buf)
		return -ENOMEM;
	...
	if (ret < 0) {
		pr_err("write error! err_code:%d, addr:0x%04X\n", ret, addr);
		return ret;
	}
...
static int ndt_stress_i2c_read(struct i2c_client *client,
			       u16 addr, u16 len, void *rxbuf)
{
	int ret = -1;
	...
	if (!rxbuf || !len) {
		pr_err("rxbuf is null, len is 0. read 0x%04X\n", addr);
		return -1;
	}
	...
	if (ret < 0) {
		pr_err("ndt_stress_i2c_read addr 0x%04X\n", addr);
		ndt_i2c_comm_unlock();
		return -1;
	}
...
static int ndt_stress_write_eeprom(struct i2c_client *client,
				   unsigned short reg, unsigned char* datbuf,
				   int ByteNo)
{
	unsigned char* buf;
	struct i2c_msg msg;
	...
	if (ret < 0)
		pr_err("i2c_master_send Error ! err_code:%d, reg: 0x%04X\n",
		ret, reg);
...
static int ndt_stress_read_eeprom(struct i2c_client *client, unsigned short reg,
				  unsigned char* datbuf, int ByteNo)
{
	struct i2c_msg msg[2];
	int ret;
...
static int ndt_stress_power_switch(bool SwFlag, bool ForceFlag)
{
	...
	if (gNdtData->power_stat == SwFlag) {
		pr_err("power mode already in power %s\n", SwFlag? "on":"down");
		return 0;
	}
	...
	else if (!gNdtData->flashprog || ForceFlag) {
		if (bdata->power_gpio >= 0) {
			gpio_set_value(bdata->power_gpio, !bdata->power_on_state);
			msleep(bdata->power_delay_ms);
		}
...
int ndt_stress_suspend(void)
{
	...
	pr_debug("start suspend!\n");

	if (ndt_Bon(gNdtData) || (!gNdtData->appear)) {
		pr_err("gNdtData is NULL & appear is false\n");
		return -1;
	}

	if (gNdtData->suspend) {
		pr_debug("device already in suspended\n");
		return -1;
	}

	if (gNdtData->flashprog) {
		pr_info("NDT do not suspend in prog flash mode !!!!\n");
		return 0;
	}
...
int ndt_stress_resume(void)
{
	...
	if (ndt_Bon(gNdtData) || !gNdtData->appear) {
		pr_err("gNdtData is NULL OR appear is false\n");
		return -1;
	}

	if (!gNdtData->suspend) {
		pr_debug("device already in resume\n");
		return -1;
	}
...
static int ndt_stress_set_gpio(struct ndt_stress_data *ndt_data)
{
	...
	retval = ndt_stress_gpio_setup(bdata->irq_gpio, true, 1, 0);
	if (retval < 0) {
		pr_err("Failed to configure attention GPIO\n");
		goto err_gpio_irq;
	}

	if (bdata->power_gpio >= 0) {
		retval = ndt_stress_gpio_setup(bdata->power_gpio,
				true, 1, !bdata->power_on_state);
		if (retval < 0) {
			pr_err("Failed to configure power GPIO\n");
			goto err_gpio_power;
		}
	}

	if (bdata->reset_gpio >= 0) {
		retval = ndt_stress_gpio_setup(bdata->reset_gpio,
				true, 0, !bdata->reset_on_state);//input
		if (retval < 0) {
			pr_err("Failed to configure reset GPIO\n");
			goto err_gpio_reset;
		}
	}
...
static void ndt_update_send_coord_work(struct work_struct *work)
{
	...
	if (gNdtData->dmode == DEBUG_MODE_LOG)
		pr_info("###### x1:0x%04X y1:0x%04X status:%u\n", x1, y1, status);
...
int ndt_get_max_coord(unsigned short max_x,  unsigned short max_y)
{
	...
	pr_debug("tp coord info, max x: %u, max y: %u\n", max_x, max_y);

	return 0;
}
EXPORT_SYMBOL(ndt_get_max_coord);

static int ndt_stress_get_base_info(unsigned short x1, unsigned short y1,
				    unsigned char status)
{
	unsigned char data_no =0;
	unsigned char data = 1;
	...
	*/
	...
	if (data_no == 8) {
		...
		if (gNdtData->dmode == DEBUG_MODE_LOG)
			pr_info("touch down:%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
				frame.press, frame.signal_sum, frame.base1,
				frame.base2, frame.base3, frame.base4,
				frame.param, frame.average,
				frame.x, frame.y, frame.is_mid_edge,
	...
	if (x_temp && y_temp) {
		...
		if ((gNdtData->frame_count > gNdtData->drop_frame_conut)) {
			...
			pr_debug("touch tremble found! frame count %d\n", gNdtData->frame_count);
...
int ndt_stress_set_value(unsigned short x1,  unsigned short y1,
			 unsigned char status)
{
	if (!gNdtData) {
		pr_err("gNdtData == NULL\n");
		return -EINVAL;
	}

	if ((gNdtData->flashprog) && (gNdtData->uniformity_init == 1))//use for test
		ndt_stress_get_base_info(x1, y1, status);

	if ((!gNdtData->appear) || (gNdtData->dmode == DEBUG_MODE_DDATA)
		||(gNdtData->flashprog))
		return -EINVAL;
	...
	if (gNdtData->ndt_coord_handle)
		queue_delayed_work(gNdtData->ndt_coord_handle, &gNdtData->send_coord_work, 0);
	else {
		pr_err("gNdtData->ndt_coord_handle = NULL!\n");
		return -EINVAL;
	}

	return 0;
}
EXPORT_SYMBOL(ndt_stress_set_value);

int ndt_stress_get_value(unsigned int *value)
{
	if (!gNdtData || !gNdtData->appear
	    || gNdtData->dmode == DEBUG_MODE_DDATA || gNdtData->flashprog)
		return -EINVAL;
...
EXPORT_SYMBOL(ndt_stress_get_value);
...
static int ndt_fwu_burn_fw(unsigned char* buf, unsigned int len)
{
	...
	pr_info("burn fw start\n");

	if (len % 128) {
		pr_err("burn len is not 128*\n");
		return -1; /* no need remove device, can not return -1 */
	}

	read_buf = (unsigned char*)kmalloc(len, GFP_KERNEL);
	if (!read_buf) {
		pr_err("kmalloc read_buf failed\n");
		return -1; /* no need remove device, can not return -1 */
	}
	do {
		pr_info("burn eeprom number: %d\n", number+1);
		...
		if (ndt_stress_write_eeprom(force_key_client, reg,
					    erase_cmd, ByteNo) > 0)
			pr_info("send cmd success---reg=0x%02x, ByteNo=%d\n",
				reg, ByteNo);
		else {
			ret = -2;
		...
		reg = IIC_EEPROM;
		ByteNo = 128;
		pr_info("start writing eeprom!\n");
		while (pos < len) {
			if (ndt_stress_write_eeprom(force_key_client, reg,
						    buf+pos, ByteNo) < 0) {
				ret = -2;
				pr_err("write eeprom failed!\n");
				goto failedloop;
			}
		...
		pr_info("write end! reg=0x%02x, ByteNo=%d\n", reg, ByteNo);
		...
		ByteNo = 128;
		while (pos < len) {
			if (ndt_stress_read_eeprom(force_key_client, reg,
						   read_buf+pos, ByteNo) < 0) {
				ret = -2;
		...
		pr_info("read reg=0x%02x, ByteNo=%d\n", reg, ByteNo);

		//check
		if (csum_partial(buf, len, 0) != csum_partial(read_buf, len, 0)) {
			for (i = 0; i < len; i++) {
				if (buf[i] != read_buf[i]) {
					pr_err("[0x%08X][0x%08X][0x%08X] burn check error!\n",
						i, buf[i], read_buf[i]);
					break;
				}
			}
	...
	if (ret) {
		pr_err("burn eeprom fail!\n");
		goto failedloop;
	} else
		pr_info("burn eeprom succeed!\n");
	...
	if (ndt_stress_write_eeprom(force_key_client, reg, erase_cmd, ByteNo) > 0)
		pr_info("reg=0x%02x, ByteNo=%d\n", reg, ByteNo);
	else {
		ret = -3;
		pr_err("exit burn failed!\n");
	}
...
static int ndt_fwu_start_reflash(void)
{
	...
	if (gNdtData->force_update)
		filename = gNdtData->update_file;
	else
		filename = NDT_FW_PATH;

	pr_info("#### force_update %u, update FW name \"%s\"",
		gNdtData->force_update, filename);

	do {
		fp = NULL;
		fp = filp_open(filename, O_RDONLY, 0);
		msleep(100 + try_count * 500);
		pr_info("try to request firmware: %d\n", try_count);
	...
	pr_info("read file_fw_ver: 0x%04X\n", ntohs(file_fw_ver));

	pr_info("gNdtData->FirmwareVer = 0x%04X\n", gNdtData->FirmwareVer);
	if ((gNdtData->FirmwareVer < 0x1A) && (!gNdtData->force_update)) {
		pr_info("Old sensor, No need to update FW !!!\n");
		goto ExitLoop;
	}

	pos = 0x0c;
	vfs_read(fp, fw_data_len, 4, &pos);
	pr_info("read fw_data_len: 0x%02x%02x%02x%02x\n", fw_data_len[0], fw_data_len[1], fw_data_len[2], fw_data_len[3]);
	...
	pr_info("read firmware crc: 0x%02x%02x\n", fw_data_len[0], fw_data_len[1]);
	firmware_crc = (unsigned short)(((unsigned int)fw_data_len[1]<<16)|(fw_data_len[0]<<24));

	if ((!gNdtData->force_update) && (gNdtData->FirmwareVer < 0x100)
	    && (ntohs(file_fw_ver) <= gNdtData->FirmwareVer)) {
		ret = 1;
		pr_info("No need to update FW\n");
		goto ExitLoop;
	}

	if ((!gNdtData->force_update) && (gNdtData->FirmwareVer == 8)) {
		ret = 1;
		pr_info("fw version = 8 and No need to update FW\n");
		goto ExitLoop;
	}
	...
	if (!fw_data) {
		ret = -1;
		pr_info("kmalloc fw_data failed!\n");
		goto ExitLoop;
	}
...
static void ndt_update_handle_work(struct work_struct *work)
{
	...
	pr_info("update handle work into\n");

	if (gNdtData->flashprog) {
		pr_err("device already in flash prog mode\n");
		return;
	}
	...
	if (ret == 1 || ret == -1) {
		...
		if (!ndt_i2c_test(0)) {
			pr_info("Update firmware success!!!\n");
			goto finishloop;
		}
	}
...
static void ndt_force_firmware_update(void)
{
	...
	queue_delayed_work(gNdtData->ndt_debug_queue, &gNdtData->update_work,
...
{
	if (gNdtData->update_finished)
		seq_printf(m, gNdtData->appear ? "yes" : "no");
	else
...
static int compatible_proc_init(void)
{
	...
	if (!res) {
		pr_err("failed to create compatible_test!\n");
		return -ENOMEM;
	}

	pr_debug("created compatible_test\n");
...
static void ndt_debug_handle_work(struct work_struct *work)
{
	...
	if (gNdtData && gNdtData->dmode && gNdtData->appear
	    && !gNdtData->suspend && !gNdtData->flashprog) {
		if (gNdtData->dmode == DEBUG_MODE_DDATA) {
			...
			if (ReadFlag == 1) {
				pr_info("Debug value: (%05d) - (%05d) = %d!!!\n",
					buffer[0], gNdtData->dvalue1,
					buffer[0] - gNdtData->dvalue1);
				mutex_lock(&(gNdtData->stress_value_mutex));
				...
				mutex_lock(&(gNdtData->stress_value_mutex));
				memcpy(gNdtData->sensor, buffer, CHANNEL_NUM * sizeof(short));
				mutex_unlock(&(gNdtData->stress_value_mutex));
				pr_info("ndt: ");
...
retry_loop:
	if (gNdtData->dmode == DEBUG_MODE_DDATA)
		queue_delayed_work(gNdtData->ndt_debug_queue,
				   &gNdtData->debug_work, msecs_to_jiffies(10));
}
				...
				size_t count, loff_t *pos)
{
	...
	if (sscanf(buffer, "%u", &input) != 1) {
		pr_err("input sscanf failed !!!\n");
		return -EINVAL;
	}

	pr_debug("input %u\n", input);

	if (!gNdtData || (input == gNdtData->dmode) || gNdtData->flashprog) {
		pr_err("dmode = input || flashprog = true\n");
		return count;
	}
	...
	pr_debug("set dmode %u\n", gNdtData->dmode);

	if (gNdtData->suspend) {
		pr_info("suspend is true\n");
		return count;
	}

	debug_mode = (gNdtData->dmode != DEBUG_MODE_NORMAL);
	ndt_stress_i2c_write(gNdtData->client, 0x60,
			     sizeof(debug_mode), &debug_mode);
...
static int debug_proc_init(void)
{
	...
	res = proc_create("driver/ndt_debug", 0x664, NULL,
			  &debug_proc_fops);
	if (!res) {
		pr_err("failed to create ndt_debug!\n");
		return -ENOMEM;
	}

	pr_debug("created ndt_debug\n");
...
static ssize_t test_proc_write(struct file *file, const char __user *buffer,
			       size_t count, loff_t *pos)
{
	pr_debug("write file name \"%s\"", buffer);

	if (!gNdtData || gNdtData->suspend) {
		pr_err("gNdtData NULL, suspend is true\n");
		return count;
	}

	if ((count > sizeof(gNdtData->update_file)) || (!count)) {
		pr_err("write count %u, update file name len %u\n",
		       (unsigned int)count,
	...
	if (copy_from_user(gNdtData->update_file, buffer, count)) {
		memset(gNdtData->update_file, 0, sizeof(gNdtData->update_file));
		pr_err("copy_from_user file name failed !!!\n");
		return count;
	}

	test_check_file_name(gNdtData->update_file,
			     sizeof(gNdtData->update_file));
...
static int test_proc_init(void)
{
	...
	res = proc_create("driver/press_test", 0x664, NULL, &test_proc_fops);
	if (!res) {
		pr_err("failed to create press_test!!!\n");
		return -ENOMEM;
	}

	pr_debug("created press_test\n");
...
{
	if (gNdtData->suspend) {
		pr_err("ndt sensor in suspend\n");
		return -ENOSPC;
	}

	if ((gNdtData->dmode == DEBUG_MODE_DDATA) || gNdtData->flashprog){
		pr_err("ndt sensor in debug or flash prog mode\n");
		return -EBUSY;
	}
...
static long dev_i2c_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		case NDT_IOC_HW_RESET:
			...
			break;
		case NDT_IOC_READ:
			pr_info("NDT_IOC_READ\n");
			if (copy_to_user((int *)arg, &data, sizeof(data)))
				return -EINVAL;
			break;
		case NDT_IOC_WRITE:
			pr_info("NDT_IOC_WRITE\n");
		...
		case NDT_IOC_INIT_UNIFORMITY:
			if (arg) {
				...
				pr_debug("init uniformity mode! drop_frame_conut %ld\n", arg);
			} else {
				gNdtData->uniformity_init = 0;
		...
		case NDT_IOC_READ_STATUS:
			...
			if (copy_to_user((char *)arg, (char *)&gpoint_info, sizeof(gpoint_info)))
				return -EINVAL;

			if (gNdtData->dmode == DEBUG_MODE_LOG)
				pr_info("touch up: %d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
					gpoint_info.press, gpoint_info.signal_sum,
					gpoint_info.base1, gpoint_info.base2,
					gpoint_info.base3, gpoint_info.base4,
					gpoint_info.param, gpoint_info.average,
					gpoint_info.x, gpoint_info.y,
					gpoint_info.is_mid_edge,
...
static int ndt_i2c_test(int delay_time)
{
	...
	do {
		mdelay(delay_time ? delay_time : 15 + i * 30);

		if (ndt_stress_i2c_read(gNdtData->client, 0x05,
					sizeof(FirmwareVer),
					(unsigned char *)&FirmwareVer) != -1
		    && FirmwareVer) {
...
static ssize_t custom_gain_proc_write(struct file *file,
				      const char __user *buffer,
				      size_t count, loff_t *pos)
{
	if (sscanf(buffer, "%u", &(gNdtData->custom_gain_value)) != 1) {
		pr_err("input custom gain failed!\n");
		return -EINVAL;
	}

	pr_info("custom gain %u\n", gNdtData->custom_gain_value);
...
static int custom_gain_proc_init(void)
{
	...
	if (!res) {
		pr_err("failed to create custom_gain_set!!!\n");
		return -ENOMEM;
	}
...
static int ndt_stress_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("NDT Driver Version: %s\n", NDT_DRIVER_VERSION);
	pr_info("NDT I2C Address: 0x%02x\n", client->addr);
	...
	if (!gNdtData) {
		pr_err("Failed to alloc mem for rmi4_data\n");
		return -ENOMEM;
	}
	...
	if (ndt_stress_set_gpio(gNdtData)) {
		pr_err("Failed to set up GPIO's\n");
		goto err_set_gpio;
	}

	if (ndt_stress_jump_wait() == -1) {
		pr_err("Failed to jump wait\n");
		//goto err_set_gpio;
	}
...
#ifdef CONFIG_NDT_USE_TEST_I2C
	if (ndt_i2c_test(0)) {
		pr_err("Failed to test ndt i2c\n");
		ret = false;
		/* tp can't init when stress sensor firmware is bad
		 * and i2c is noisy. we must pown down */
		ndt_stress_power_switch(false, false);
#ifdef CONFIG_NDT_USE_UPDATE
		/* delay 10s force update firmware when
		 * filesystem have mounted */
