...
#ifdef GF_DEBUG
#define gf_dbg pr_warn
#define FUNC_ENTRY() pr_warn("entry\n")
#define FUNC_EXIT() pr_warn("exit\n")
#else
...
static void gf_enable_irq(struct gf_dev *gf_dev)
{
	if (gf_dev->irq_enabled) {
		pr_warn("IRQ is already enabled\n");
		return;
	}
...
{
	if (!gf_dev->irq_enabled) {
		pr_warn("IRQ is already disabled\n");
		return;
	}
...
static void spi_clock_set(struct gf_dev *gf_dev, int speed)
{
	...
	if (rate < 0) {
		pr_info("no match found for requested clock frequency: %d\n",
			speed);
		return;
	}
...
static int gfspi_ioctl_clk_init(struct gf_dev *data)
{
	...
	if (IS_ERR_OR_NULL(data->core_clk)) {
		pr_err("fail to get core_clk\n");
		return -1;
	}

	data->iface_clk = clk_get(&data->spi->dev, "iface_clk");
	if (IS_ERR_OR_NULL(data->iface_clk)) {
		pr_err("fail to get iface_clk\n");
...
static int gfspi_ioctl_clk_enable(struct gf_dev *data)
{
	...
	if (err) {
		pr_err("fail to enable core_clk\n");
		return -1;
	}

	err = clk_prepare_enable(data->iface_clk);
	if (err) {
		pr_err("fail to enable iface_clk\n");
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (!gf_dev->device_available &&
	    cmd != GF_IOC_POWER_ON && cmd != GF_IOC_POWER_OFF) {
		pr_warn("sensor cmd 0x%x is unavailable while powered-off!\n",
			cmd);
		return -ENODEV;
	}
	...
	case GF_IOC_SETSPEED:
#ifdef AP_CONTROL_CLK
		ret = __get_user(speed, (u32 __user *) arg);
		if (ret) {
		...
		break;
	case GF_IOC_RESET:
		ret = __get_user(delay_ms, (u32 __user *)arg);
		if (ret) {
			pr_warn("failed to set reset delay from user "
				"(ret = %d)\n", ret);
			break;
		}
					...
					unsigned long val, void *data)
{
	...
	if (gf_dev && evdata && evdata->data && val == FB_EARLY_EVENT_BLANK) {
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
	...
	if (input_register_device(gf_dev->input))
		pr_warn("failed to register GF as input device <-- lol\n");
}
...
#endif
		if (!ret) {
			ret = enable_irq_wake(gf_dev->irq);
			if (ret)
				dev_err(&gf_dev->input->dev,
					"failed to enable IRQ wake, ret = %d\n",
					ret);
			else
				gf_dev->irq_enabled = 1;
			gf_disable_irq(gf_dev);
