			      ...
			      const char *buf, size_t count)
{
	...
	for (i = 0; i < pt_count; i += 2) {
		...
		ADBG("write add=%d, val=%d", val[i], val[i+1]);
	}
			      ...
			      const char *buf, size_t count)
{
	...
	while ((p = strsep(&ptr_data, ","))) {
		...
		if (pt_count % 3 == 1) {
			...
			ADBG("add = %02x%02x, val=%02x%02x%02x",
			      ...
			      const char *buf, size_t count)
{
	...
	ret += ak49xx_ram_write(debugak49xx, vat, page, start, pt_count, val);
		//for slim_spi 2014-11-17 14:16:45
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < 16; i++) {
		ret = ak49xx_reg_read(&debugak49xx->core_res,
				      MIR1_REGISTER_1 + i);
		if (ret < 0) {
			pr_err("read register error\n");
			break;
...
static DEVICE_ATTR(mir_data, 0444, mir_data_show, NULL);
#endif
...
{
	ADBG("mclk_enable = %u, dapm = %d", mclk_enable, dapm);
...
static int __ak4961_mclk_enable(struct ak4961_priv *ak4961, int mclk_enable)
{
	...
	if (mclk_enable) {
		...
		if (ret) {
			pr_err("ext clk enable failed\n");
			ak4961->ext_clk_users--;
			return ret;
		}
	} else {
		ak4961->ext_clk_users--;
		if (!ak4961->ext_clk_users)
			     ...
			     int enable, bool dapm)
{
	...
	if (ak4961->cdc_ext_clk) {
		ADBG("mclk_enable = %u, dapm = %d", enable, dapm);
		return __ak4961_mclk_enable(ak4961, enable);
	} else {
		ADBG("Cannot turn on MCLK");
		return -EINVAL;
	}
}
			  ...
			  struct snd_soc_dai *dai)
{
	...
	ADBG("substream = %s, stream = %d",
		substream->name, substream->stream);

	if (ak4961_core && ak4961_core->dev && ak4961_core->dev->parent)
		pm_runtime_get_sync(ak4961_core->dev->parent);

	ADBG("ak4961_dsp_mode = %d, dai_id = %d", ak4961_dsp_mode, dai->id);
	return 0;
}
			    ...
			    struct snd_soc_dai *dai)
{
	...
	ADBG("ak4961_dsp_mode = %d, dai_id = %d", ak4961_dsp_mode, dai->id);
	if (ak4961_dsp_mode && dai->id == SB3_CAP) {
		...
		struct snd_soc_dai *dai)
{
	...
	ADBG("dai_name = %s, DAI-ID %x, rate %d, num_ch %d, format %d",
	...
	case 192000:
		...
		break;
	default:
		pr_err("Invalid sampling rate %d\n", params_rate(params));
		return -EINVAL;
	}
	...
	case SNDRV_PCM_FORMAT_S32_LE:
		...
		break;
	default:
		pr_err("Invalid RX format %u\n", params_format(params));
		return -EINVAL;
	}

	ADBG("dai_name = %s, DAI-ID %x, bit_width %d",
...

{
	...
	ADBG("dai_name = %s, DAI-ID = %x, tx_ch = %d, "
		"rx_ch = %d, ak4961->intf_type = %d",
		dai->name, dai->id, tx_num, rx_num, ak4961->intf_type);

	//for slim_spi 2014-11-17 14:17:46
	if (ak4961->intf_type == AK49XX_INTERFACE_TYPE_SLIMBUS ||
	    ak4961->intf_type == AK49XX_INTERFACE_TYPE_SLIMBUS_SPI)
...

{
	...
	case SB3_PB:
		if (!rx_slot || !rx_num) {
			pr_err("Invalid rx_slot %p or rx_num %p\n",
				rx_slot, rx_num);
			return -EINVAL;
		}
	...
	case SB3_CAP:
		if (!tx_slot || !tx_num) {
			pr_err("Invalid tx_slot %p or tx_num %p\n",
				tx_slot, tx_num);
			return -EINVAL;
		}
		list_for_each_entry(ch, &ak4961_p->dai[dai->id].ak49xx_ch_list,
		...
		struct snd_soc_dai *dai)
{
	...
	ADBG("dai_name = %s, DAI-ID %x, rate %d, num_ch %d",
		dai->name, dai->id, params_rate(params),
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!ak4961->ak4961_dsp_uplink_status &&
	    !ak4961->ak4961_dsp_downlink_status) {
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4961_priv *ak4961 = snd_soc_codec_get_drvdata(codec);
	//for slim_spi 2014-11-17 14:23:30
	struct ak49xx *ak49xx = dev_get_drvdata(codec->dev->parent);
	...
	u8 pram_index, cram_index, aram_index,
		ex_pram_index, ex_cram_index, ex_aram_index;
	...
	if (new_dsp_mode != DSP_MODE_OFF) {
		...
		ADBG("kcontrol: %p, ucontrol: %p", kcontrol, ucontrol);
		ADBG("interface: %u", interface);
		ADBG("new DSP mode: %u", new_dsp_mode);
		ADBG("pram_index: %u, cram_index: %u, aram_index: %u",
			pram_index, cram_index, aram_index);

		if (pram_index != ex_pram_index && ak4961->pram_firmware[pram_index]) {
			pram_fwbuf = kmemdup(ak4961->pram_firmware[pram_index]->data,
					ak4961->pram_firmware[pram_index]->size, GFP_KERNEL);
			if (!pram_fwbuf) {
				pr_err("MEM Allocation for PRAM failed\n");
				return -ENOMEM;
			}

			ADBG("PRAM memory allocated at %p (size: %zu)",
				pram_fwbuf,
				ak4961->pram_firmware[pram_index]->size);
			...
			ADBG("pram_size = %zu", pram_size);
		}

		if (cram_index != ex_cram_index && ak4961->cram_firmware[cram_index]) {
			cram_fwbuf = kmemdup(ak4961->cram_firmware[cram_index]->data,
					ak4961->cram_firmware[cram_index]->size, GFP_KERNEL);
			if (!cram_fwbuf) {
				pr_err("MEM Allocation for CRAM failed\n");
				return -ENOMEM;
			}

			ADBG("CRAM memory allocated at %p (size: %zu)",
				cram_fwbuf,
				ak4961->cram_firmware[cram_index]->size);
			...
			ADBG("cram_size = %zu", cram_size);
		}

		if (aram_index != ex_aram_index && ak4961->aram_firmware[aram_index]) {
			aram_fwbuf = kmemdup(ak4961->aram_firmware[aram_index]->data,
					ak4961->aram_firmware[aram_index]->size, GFP_KERNEL);
			if (!aram_fwbuf) {
				pr_err("MEM Allocation for ARAM failed\n");
				return -ENOMEM;
			}

			ADBG("ARAM memory allocated at %p (size: %zu)",
				cram_fwbuf,
				ak4961->aram_firmware[aram_index]->size);
		...
		if (pram_fwbuf) {
			...
			ADBG("PRAM: ret = ak49xx_ram_write() = %d", ret);

			if (interface != AK49XX_INTERFACE_TYPE_SLIMBUS) {
				ret += ak49xx_bulk_read(core_res,
						CRC_RESULT_H8, 2, crc);

				ADBG("PRAM: ret += ak49xx_bulk_read() = %d", ret);
				ADBG("PRAM: crc[0] = %u, crc[1] = %u",
					crc[0], crc[1]);
				ADBG("PRAM: pram_fwbuf[pram_size] = %u, "
					"pram_fwbuf[pram_size + 1] = %u",
					pram_fwbuf[pram_size],
					pram_fwbuf[pram_size + 1]);

				if (((pram_fwbuf[pram_size] << 8) +
				      pram_fwbuf[pram_size + 1]) !=
				    ((crc[0] << 8) + crc[1])) {
					/* jcadduono modification ***
					 * ignore PRAM CRC failure, not sure
					 * why it is not matching, appears to
					 * succeed?
					ret = -EIO;
					*/
					pr_err("PRAM download CRC failed\n");
				}
			}
			...
			if (ret) {
				pr_err("PRAM download failed (ret = %d)\n", ret);
				err++;
			}
		...
		if (cram_fwbuf) {
			...
			ADBG("CRAM: ret = ak49xx_ram_write() = %d", ret);

			if (interface != AK49XX_INTERFACE_TYPE_SLIMBUS) {
				ret += ak49xx_bulk_read(core_res,
						CRC_RESULT_H8, 2, crc);

				ADBG("CRAM: ret += ak49xx_bulk_read() = %d", ret);
				ADBG("CRAM: crc[0] = %u, crc[1] = %u",
					crc[0], crc[1]);
				ADBG("CRAM: cram_fwbuf[cram_size] = %u, "
					"cram_fwbuf[cram_size + 1] = %u",
					cram_fwbuf[cram_size],
					cram_fwbuf[cram_size + 1]);
				...
				if (((cram_fwbuf[cram_size] << 8) +
				    ...
				    ((crc[0] << 8) + crc[1])) {
					ret = -EIO;
					pr_err("CRAM download CRC failed\n");
				}
			}
			...
			if (ret) {
				pr_err("CRAM download failed (ret = %d)\n", ret);
				err++;
			}
		}
		...
		if (aram_fwbuf) {
			...
			ADBG("ARAM: ret = ak49xx_ram_write() = %d", ret);

			if (interface != AK49XX_INTERFACE_TYPE_SLIMBUS) {
				ret += ak49xx_bulk_read(core_res,
						CRC_RESULT_H8, 2, crc);

				ADBG("ARAM: ret += ak49xx_bulk_read() = %d", ret);
				ADBG("ARAM: crc[0] = %u, crc[1] = %u",
					crc[0], crc[1]);
				ADBG("ARAM: aram_fwbuf[aram_size] = %u, "
					"aram_fwbuf[aram_size + 1] = %u",
					aram_fwbuf[aram_size],
					aram_fwbuf[aram_size + 1]);

				if (((aram_fwbuf[aram_size] << 8) +
				     aram_fwbuf[aram_size + 1]) !=
				    ((crc[0] << 8) + crc[1])) {
					ret = -EIO;
					pr_err("ARAM download CRC failed\n");
				}
			}
			...
			if (ret) {
				pr_err("ARAM download failed (ret = %d)\n", ret);
				err++;
			}
		}
	...
	if (!err) {
		...
		ADBG("new_dsp_mode = %u", new_dsp_mode);
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	snd_soc_write(codec, RUN_STATE_DATA_1,
			mic_gain_table2[karaoke_mic_gain]>>16);
	snd_soc_write(codec, RUN_STATE_DATA_2,
			(mic_gain_table2[karaoke_mic_gain]>>8) % 0x100);
	snd_soc_write(codec, RUN_STATE_DATA_3,
			mic_gain_table2[karaoke_mic_gain] % 0x100);
	snd_soc_write(codec, RUN_STATE_DATA_4,
			mic_gain_table2[karaoke_mic_gain]>>16);
	snd_soc_write(codec, RUN_STATE_DATA_5,
			(mic_gain_table2[karaoke_mic_gain]>>8) % 0x100);
	snd_soc_write(codec, RUN_STATE_DATA_6,
			mic_gain_table2[karaoke_mic_gain] % 0x100);
	snd_soc_write(codec, CRAM_RUN_EXE, 0x01);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	ADBG("howtochange = %d", howtochange);
					     ...
					     struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((internal_ahpf_sensitivity % 2))
		val[10] = 0x80;
	else
		val[10] = 0x00;
	val[11] = 0x00;
...
static const struct soc_enum voice_sync_switch_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(voice_sync_switch_text),
			    voice_sync_switch_text);
...
static const struct soc_enum slimbus_interface_reg_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(slimbus_interface_reg_text),
			    slimbus_interface_reg_text);
					    ...
					    struct snd_ctl_elem_value *ucontrol)
{
	...
	case 2:
		ret = ak49xx_interface_reg_read(ak49xx, AK496X_SLIM_PGD_PORT0_ARRAY);
		ADBG("sliminter reg 0x00 ret = %x, cnt = %d", ret, cnt);

		ak49xx_interface_reg_write(ak49xx, AK496X_SLIM_PGD_PORT0_ARRAY, 0x0A);
		do {
			ret = ak49xx_interface_reg_read(ak49xx, AK496X_SLIM_PGD_PORT0_ARRAY);
			ADBG("sliminter reg 0x00 ret = %x cnt = %d\n", ret, cnt);
			msleep(1000);
			cnt++;
...
static const struct soc_enum ak4961_power_test_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ak4961_power_test_text),
			    ak4961_power_test_text);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	case 2:
	case 3:
		ak49xx_power_test(ak49xx, status);
		break;
	default:
		break;
	}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	ADBG("wname %s cname %s value %u shift %d item %ld",
		widget->name, ucontrol->id.name, ak4961_p->tx_port_value,
		widget->shift, ucontrol->value.integer.value[0]);

	mutex_lock(&codec->mutex);
	//for slim_spi 2014-11-17 14:37:44
	if ((ak4961_p->intf_type != AK49XX_INTERFACE_TYPE_SLIMBUS) &&
	    (ak4961_p->intf_type != AK49XX_INTERFACE_TYPE_SLIMBUS_SPI) &&
	    (dai_id != SB1_CAP)) {
		dev_err(codec->dev, "invalid AIF for I2S mode\n");
	...
	case SB3_CAP:
		...
		if (enable && !(ak4961_p->tx_port_value & 1 << port_id)) {
			//for slim_spi 2014-11-17 14:38:47
			if (ak4961_p->intf_type == AK49XX_INTERFACE_TYPE_SLIMBUS ||
			    ak4961_p->intf_type ==AK49XX_INTERFACE_TYPE_SLIMBUS_SPI)
			...
			if (ak49xx_tx_vport_validation(vtable, port_id,
					ak4961_p->dai, NUM_CODEC_DAIS)) {
				dev_dbg(codec->dev,
					"TX%u is used by other virtual port\n",
					port_id + 1);
				mutex_unlock(&codec->mutex);
				return 0;
			}
			...
			if (enable)
				dev_dbg(codec->dev,
					"TX%u port is used by this virtual port\n",
					port_id + 1);
			else
				dev_dbg(codec->dev,
					"TX%u port is not used by this virtual port\n",
					port_id + 1);
	...
	ADBG("name %s sname %s updated value %u shift %d",
		widget->name, widget->sname,
		ak4961_p->tx_port_value, widget->shift);
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	ADBG("wname %s cname %s value %u shift %d item %u",
		widget->name, ucontrol->id.name, ak4961_p->rx_port_value,
		widget->shift, ucontrol->value.enumerated.item[0]);

	ak4961_p->rx_port_value = ucontrol->value.enumerated.item[0];

	mutex_lock(&codec->mutex);

	//for slim_spi 2014-11-17 14:40:46
	if (ak4961_p->intf_type != AK49XX_INTERFACE_TYPE_SLIMBUS &&
	    ak4961_p->intf_type != AK49XX_INTERFACE_TYPE_SLIMBUS_SPI &&
	    ak4961_p->rx_port_value > 2) {
		dev_err(codec->dev, "invalid AIF for I2S mode\n");
		goto err;
	}
	...
	case 1:
		if (ak49xx_rx_vport_validation(port_id +
			...
			dev_dbg(codec->dev,
				"RX%u is used by current requesting AIF_PB itself\n",
				port_id + 1);
			goto rtn;
		}
...
rtn:
	...
	snd_soc_dapm_mux_update_power(widget->dapm, kcontrol,
				      ak4961_p->rx_port_value, e, update);
	return 0;
err:
	mutex_unlock(&codec->mutex);
	return -EINVAL;
				      ...
				      struct snd_kcontrol *kcontrol, int event)
{
	...
	ADBG("event called! codec name %s, num_dai %d, stream name %s, event %d",
		w->codec->component.name, w->codec->component.num_dai,
		w->sname, event);

	/* Execute the callback only if interface type is slimbus */
	//for slim_spi 2014-11-17 14:42:08
	if (ak4961_p->intf_type != AK49XX_INTERFACE_TYPE_SLIMBUS &&
	    ak4961_p->intf_type != AK49XX_INTERFACE_TYPE_SLIMBUS_SPI)
		return 0;

	dai = &ak4961_p->dai[w->shift];
	ADBG("w->name %s, w->shift %d, event %d", w->name, w->shift, event);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (ret < 0) {
			ret = ak49xx_disconnect_port(core,
						     &dai->ak49xx_ch_list,
						     dai->grph);
			ADBG("Disconnect RX port, ret = %d", ret);
		}
		break;
	}
	return ret;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	ADBG("event called! codec name %s, num_dai %d, stream name %s",
		w->codec->component.name, w->codec->component.num_dai, w->sname);

	/* Execute the callback only if interface type is slimbus */
	//for slim_spi 2014-11-17 14:42:52
	if (ak4961_p->intf_type != AK49XX_INTERFACE_TYPE_SLIMBUS &&
	    ak4961_p->intf_type != AK49XX_INTERFACE_TYPE_SLIMBUS_SPI)
		return 0;

	ADBG("w->name %s, event %d, w->shift %d", w->name, event, w->shift);
				   ...
				   struct snd_kcontrol *kcontrol, int event)
{
	...
	if (ret < 0) {
		pr_err("Invalid AMIC line on the codec\n");
		return -EINVAL;
	}
				    ...
				    struct snd_kcontrol *kcontrol, int event)
{
	...
	if (ret < 0) {
		pr_err("Invalid DMIC line on the codec\n");
		return -EINVAL;
	}
...
static irqreturn_t ak4961_jde_irq(int irq, void *data)
{
	...
	if (val < 0) {
		...
		dev_err(codec->dev,
			"Failed to read JACK_DETECTION_STATUS: %d\n", val);
		return IRQ_HANDLED;
	}
	...
	if (report != last_report_sw) {
		...
		if (report == 1) {
			...
			dev_dbg(codec->dev, "delay btn det %d\n",
...
//ZTE_weizj 20150601 fix:Fake button press while inserting headset
			...
			schedule_delayed_work(&priv->mbhc_cfg.reject_btn_dwork,
					      msecs_to_jiffies(2000));
#else
			cancel_delayed_work(&priv->mbhc_cfg.btn_work);
			queue_delayed_work(priv->mbhc_cfg.btn_workqueue,
					   &priv->mbhc_cfg.btn_work,
					   msecs_to_jiffies(2500));
#endif
//ZTE_weizj 20150601 end
		}
		dev_dbg(codec->dev, "report %d, mic_det_repeat %d\n",
			report, mic_det_repeat);
	}
...
static irqreturn_t ak4961_rce_irq(int irq, void *data)
{
	...
	if (val & 0x01) {
		...
		if (mic_level < 0) {
			snd_soc_write(codec, DETECTION_EVENT_RESET, 0x01);
			dev_err(codec->dev,
				"Failed to read MIC_LEVEL_DETECTION: %d\n",
				mic_level);
			return IRQ_HANDLED;
		}
...
//ZTE_weizj 20150601 fix:Fake button press while inserting headset
		...
		if (report == 226 && priv->mbhc_cfg.reject_btn) {
			ADBG("do not report btn det reject(%d)",
				priv->mbhc_cfg.reject_btn);
			//snd_soc_write(codec, DETECTION_EVENT_RESET, 0x01);
...
//ZTE_weizj 20150601 end
			...
			dev_dbg(codec->dev, "mic_level %d report %d\n", mic_level, report);
		}//weizj end

	} else if (!report && last_report_key) {
...
static irqreturn_t ak4961_vad_irq(int irq, void *data)
{
	...
	if (val & 0x04)
		dev_dbg(codec->dev, "report active\n");
...
static void ak4961_reject_btn_fn(struct work_struct *work)
{
	...
	ADBG("release btn det reject_btn(%d)", mbhc->reject_btn);
}
		     ...
		     const struct ak4961_mbhc_config *cfg)
{
	...
	case AK4961_MCLK_RATE_12288KHZ:
		...
		ADBG("MCLK: clock rate using %dHz", cfg->mclk_rate);
		break;
...
// ZTE_weizj 20150601 fix:Fake button press while inserting headset
	...
	if (!ak4961->mbhc_cfg.btn_workqueue)
		pr_err("can't create work queue\n");
...
//ZTE_weizj 20150601 end
	...
	if (rc < 0) {
		pr_err("Error in switch_dev_register, rc = %d\n", rc);
	...
	if (rc) {
		pr_err("Error in input_register_device, rc = %d\n", rc);
...
static void ak4961_init_slim_slave_cfg(struct snd_soc_codec *codec)
{
	...
	ADBG("slimbus logical address 0x%llx\n", eaddr);
}
			    ...
			    enum afe_config_type config_type)
{
	...
	default:
		pr_err("Unknown config_type 0x%x\n", config_type);
	}
...
static void ak4961_aram_ready(const struct firmware *fw, void *context)
{
	...
	ADBG("ak4961->aram_load_index = %u", ak4961->aram_load_index);
...
static void ak4961_pram_ready(const struct firmware *fw, void *context)
{
	...
	ADBG("ak4961->pram_load_index = %u", ak4961->pram_load_index);
...
static void ak4961_cram_ready(const struct firmware *fw, void *context)
{
	...
	ADBG("ak4961->cram_load_index = %u", ak4961->cram_load_index);
...
static int ak4961_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (!of_find_property(ak49xx->dev->of_node, "clock-names", NULL)) {
		dev_dbg(ak49xx->dev, "codec not using audio-ext-clk driver\n");
	} else {
		cdc_ext_clk = clk_get(ak49xx->dev, "cdc_clk");
		if (IS_ERR(cdc_ext_clk)) {
			dev_err(codec->dev, "clk get cdc_ext_clk failed\n");
	...
	ptr = kmalloc((sizeof(ak4961_rx_chs) + sizeof(ak4961_tx_chs)),
		      GFP_KERNEL);
	if (!ptr) {
		pr_err("no mem for slim chan ctl data\n");
		ret = -ENOMEM;
		goto err_nomem_slimch;
	}
	...
	//for slim_spi 2014-11-17 14:54:44
	if (ak4961->intf_type == AK49XX_INTERFACE_TYPE_SLIMBUS ||
		ak4961->intf_type == AK49XX_INTERFACE_TYPE_SLIMBUS_SPI) {
...
static int ak4961_write(struct snd_soc_codec *codec,
...
static int ak4961_probe(struct platform_device *pdev)
{
	...
	ak4961 = devm_kzalloc(&pdev->dev, sizeof(struct ak4961_priv),
			      GFP_KERNEL);
	if (!ak4961) {
		dev_err(&pdev->dev, "cannot create memory for ak4961\n");
		return -ENOMEM;
	}
...
/* zte weizhijun add headset detect /proc/hs 2013-12-17 end*/
	...
	if (ak49xx_get_intf_type() == AK49XX_INTERFACE_TYPE_SLIMBUS ||
		...
		ADBG("ak4961_probe: register I2S dai");
		ret = snd_soc_register_codec(&pdev->dev, &soc_codec_dev_ak4961,
